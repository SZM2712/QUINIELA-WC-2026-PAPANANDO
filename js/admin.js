// Panel de Admin: autenticacion (Firebase Auth), carga de resultados reales, gestion de participantes.
import {GS,MU,TEAMS,R32,R16P,R16V,QFP_REAL,QFV,SFP,SFV,PPTS} from './data.js';
import {AppState} from './state.js';
import {stG,stS,stU,stD,getAllUsers,addIdx,delIdx,adminSignIn,adminSignOut,isAdminUid,getCurrentUid,rawGetAll} from './firebase.js';
...
export async function chkAdm(){
  var emailEl=document.getElementById('ap-email'),passEl=document.getElementById('ap-pass'),errEl=document.getElementById('aerr');
  ...
  try{
    var uid=await adminSignIn(email,pass);
    var ok=await isAdminUid(uid);
    if(!ok){
      errEl.textContent='Tu cuenta existe pero no está autorizada como admin. Revisa el paso 3 de SETUP_FIREBASE.md (agregar tu UID a pap26_admins).';
      await adminSignOut();
      return;
    }
    AppState.adm=true;errEl.textContent='';
    initRS();await loadRD();renderAdmin();
  }catch(e){errEl.textContent='Credenciales inválidas o no configuradas ('+e.message+').';}
}
export async function adminLogout(){
  AppState.adm=false;await adminSignOut();renderAdmin();
}
...
export async function renderAdmin(){
  ...
  if(!AppState.adm){
    wrap.innerHTML='<div class="abox"><div class="atitle">Admin</div>'+
      '<label class="ll">Correo</label><input class="li" id="ap-email" type="email" autocomplete="username" placeholder="admin@ejemplo.com">'+
      '<label class="ll">Contraseña</label><input class="li" id="ap-pass" type="password" autocomplete="current-password" onkeydown="if(event.key===\'Enter\')chkAdm()">'+
      '<button class="lbtn" style="background:var(--purple)" onclick="chkAdm()">INGRESAR</button><div class="lerr" id="aerr"></div>'+
      '<div style="font-size:9px;color:var(--muted);margin-top:8px;">Requiere Firebase Auth configurado - ver SETUP_FIREBASE.md</div></div>';
    return;
  }
  await loadRD();
